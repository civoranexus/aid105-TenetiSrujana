...
if not st.session_state.logged_in:
    ...
    with c2:
        ...
        # 🔥 FIXED HERE (scheme_state ➜ state)
        state = st.selectbox(
            "State",
            ["Select your state"] + sorted(df["state"].unique())
        )
...
h1.markdown(
    "<div class='flow'><h3>👤 Step 1</h3><p>Enter your personal details</p></div>",
    unsafe_allow_html=True
)
h2.markdown(
    "<div class='flow'><h3>🧠 Step 2</h3><p>AI evaluates eligibility & urgency</p></div>",
    unsafe_allow_html=True
)
h3.markdown(
    "<div class='flow'><h3>🚀 Step 3</h3><p>Apply confidently before deadlines</p></div>",
    unsafe_allow_html=True
)
...
with c1:
    urgency_data = filtered["deadline"].apply(
        lambda d: "HIGH" if (d - datetime.now()).days < 30
        else "MEDIUM" if (d - datetime.now()).days < 60
        else "LOW"
    ).value_counts()
...
with c2:
    st.line_chart(
        filtered.set_index("scheme_name")["estimated_benefit"]
    )

st.info("📈 Insight: Applying early to high-urgency schemes improves approval chances.")
...
with st.expander("Is this an official government website?"):
    st.write("No. This is an academic AI assistance platform built for educational purposes.")

with st.expander("How are schemes recommended?"):
    st.write("Schemes are filtered based on income, category, state, and deadline urgency.")

with st.expander("Why does urgency matter?"):
    st.write("Schemes with closer deadlines may close soon, so applying early increases chances.")

with st.expander("Is my personal data stored?"):
    st.write("No. This app does not store or share any personal information.")
